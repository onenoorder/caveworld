...
import { CompositeGoal, TurnGoal, WalkGoal, ClimbGoal } from '_goals/index';
...
export class FollowPathGoal extends CompositeGoal {
  ...
  Activate() {
    ...
    if (this.path.length > 0 && this.path[this.path.length - 1].EqualsPosition(this.dwarf.position)) {
      ...
      let previousPosition = this.dwarf.position.clone();
      for (let index = this.path.length - 1; index >= 1; index--) {
        ...
        if (previousPosition.x !== current.position.x) {
          this.subGoals.Push(new WalkGoal(this.dwarf, current.position.clone()));
        } else if (index === this.path.length - 1) {
          this.subGoals.Push(new ClimbGoal(this.dwarf, current.position.clone()));
        } else {
          this.subGoals.Push(new ClimbGoal(this.dwarf, previousPosition));
          this.subGoals.Push(new WalkGoal(this.dwarf, current.position.clone()));
        }
